...
module.exports.verifyCredentials = function (req, res, next) {
  passport.authenticate('local', function (err, user, info) {
    ...
    if (!user) { return res.send({message: 'Error authentcating. Username or password is incorrect'}) }
...
module.exports.findUserByUsername = function (username, callback) {
  User.findOne(
    {username: username},
    function (err, user) {
      if (err) { callback(err, null) } else if (user) {
        var message = {
          message: 'User exists',
          userId: user._id
        }
...
module.exports.verifyCredentialsApp = function (req, res, next) {
  ...
  passport.authenticate('local', function (err, user, info) {
    ...
    if (!user) { return res.send({message: 'Error authentcating. Username or password is incorrect', success: false}) }
